...
namespace JudgeSystem.Compilers
{
    public class JavaCompiler : ICompiler
    {
        public CompileResult Compile(string fileName, string workingDirectory, IEnumerable<string> sources = null)
        {
            ...
            string arguments = $"{GlobalConstants.ConsoleComamndPrefix} cd {workingDirectory} && {CompilationSettings.SetJavaCompilerPathCommand} & javac {sourceFile}";
            //string arguments = $"{GlobalConstants.ConsoleComamndPrefix} cd {workingDirectory} && javac {sourceFile}";

            var compiler = new Compiler();
            CompileResult result = compiler.Compile(arguments, GlobalConstants.ConsoleFile);
